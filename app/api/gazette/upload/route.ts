...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileUrl = `/uploads/gazettes/${Date.now()}-${file.name}`
    ...
    const result = await db.collection<GazetteUpload>('gazette_uploads').insertOne(gazetteUpload)
...
export async function GET() {
  try {
    const db = await getDatabase()
    const uploads = await db.collection<GazetteUpload>('gazette_uploads')
      .find({})
      .sort({ uploadedAt: -1 })
      .toArray()
