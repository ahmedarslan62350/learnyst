...
import { Upload, CheckCircle, Clock, AlertCircle, Eye, Trash2, Download } from 'lucide-react'
...
export default function AdminGazetteClient() {
  ...
  const updateUploadStatus = async (uploadId: string, status: 'approved' | 'rejected', notes?: string) => {
  ...
  const processedUploads = uploads.filter(upload => upload.status !== 'pending')
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-800">Gazette Management</h1>
              <p className="text-slate-600">Review and approve gazette uploads from users</p>
            </div>
          </div>
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {pendingUploads.map((upload) => (
                <Card key={upload._id} className="shadow-lg border-orange-200 bg-orange-50">
                  <CardHeader className="pb-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        {getStatusIcon(upload.status)}
                        <CardTitle className="text-lg">{upload.boardName}</CardTitle>
                      </div>
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-slate-600 truncate">{upload.fileName}</p>
                      </div>
                      <div>
                        <p className="font-medium text-slate-700">Upload Date:</p>
                        <p className="text-slate-600">{new Date(upload.uploadedAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex space-x-3 pt-4 border-t border-orange-200">
                      <Button
                        onClick={() => updateUploadStatus(upload._id, 'approved', adminNotes[upload._id])}
                      ...
                      <Button
                        onClick={() => updateUploadStatus(upload._id, 'rejected', adminNotes[upload._id])}
                        disabled={processingId === upload._id}
                        variant="destructive"
                        className="flex-1"
                      >
          ...
          {processedUploads.length > 0 ? (
              ...
              {processedUploads.map((upload) => (
                <Card key={upload._id} className="shadow-lg border-slate-200">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {upload.adminNotes && (
                      <div className="text-sm">
                        <p className="font-medium text-slate-700">Admin Notes:</p>
                        <p className="text-slate-600 bg-slate-50 p-2 rounded text-xs">{upload.adminNotes}</p>
                      </div>
                    )}

                    <div className="flex justify-between items-center pt-3 border-t border-slate-200 text-xs text-slate-500">
                      <span>Processed: {upload.processedAt ? new Date(upload.processedAt).toLocaleDateString() : 'N/A'}</span>
            ...
            <Card className="shadow-lg border-slate-200">
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-lg font-semibold text-slate-600 mb-2">No Processed Uploads</h3>
                <p className="text-slate-500">Processed uploads will appear here after review.</p>
              </CardContent>
            </Card>
          )}
        </div>
